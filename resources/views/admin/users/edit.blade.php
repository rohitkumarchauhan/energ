                    ...
                    <form class="form-horizontal" method="post" action="{{ route('users.update',$user->id) }}"
                        ...
                        <div class="card-body">
                            <div class="form-group row">
                                ...
                                <div class="col-sm-9">
                                    <input type="text" name="name" class="form-control" id="fname" placeholder="" value="{{ $user->name or '' }}" required>
                                </div>
                            </div>
                            <div class="form-group row">
                                <label for="lname" class="col-sm-3 text-right control-label col-form-label">Email</label>
                                <div class="col-sm-9">
                                    <input type="email" name="email" class="form-control" id="lname" placeholder="" value="{{ $user->email or '' }}" required>
                                </div>
                            </div>
                                ...
                                <div class="col-md-9">
                                    <select class="select2 form-control custom-select" style="width: 100%; height:36px;" required>
                                        @foreach($roles as $key =>$val)
                                            <option value="{{ $key }}" {{  ($key==$user->role_id)?'selected':''   }} >{{ $val }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-md-6">
                                    ...
                                    <div class="img-container has-image-container" style="@if(empty($user->profileImage)) display: none @endif">
                                        <img id="image" src="{{asset('images/thumbnail/profile').'/thumb-'.$user->profileImage}}" alt="Picture">
                                    </div>
                                </div>
                            </div>
